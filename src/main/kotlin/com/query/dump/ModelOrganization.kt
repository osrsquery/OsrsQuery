...
object ModelOrganization {
    ...
    fun init() {
       ...
       val pb = progress("Dumping Objects",objects.size.toLong())
        objects.forEach {
            ...
            val dir = FileUtil.getFile("/modelsOrganised/objects/", if(exists.contains(name)) "${name}-${it.id}" else name)
        ...
        npcs.forEach {
            val name = formatName(it.name,it.id)
            val dir = FileUtil.getFile("/modelsOrganised/npcs/", if(exists.contains(name)) "${name}-${it.id}" else name)
            ...
            if(it.chatheadModels != null) {
                it.chatheadModels!!.forEach { modelId ->
                    val model = getFiles(modelId)
                    val dir1 = FileUtil.getDir("/modelsOrganised/npcs/${if(exists.contains(name)) "${name}-${it.id}" else name}/chatheads/")
                    dir1.mkdirs()
    ...
    private fun getFiles(id : Int) : Pair<File,File> {
        return Pair(
            FileUtil.getFile("/modelsObj/", "$id.obj"),
            FileUtil.getFile("/modelsObj/", "$id.mtl")
        )
    }
    ...
    private fun writeItemFiles(id : Int,modelID: Int,name : String,type : String, gender : String) {
        ...
        val dir = FileUtil.getDir("/modelsOrganised/items/${formatName(name,id)}")
        val files =  Pair(
            FileUtil.getFile("/modelsObj/", "$modelID.obj"),
            FileUtil.getFile("/modelsObj/", "$modelID.mtl")
        )
